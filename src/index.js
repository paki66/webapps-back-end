...
app.patch("/users/status/:email", async (req, res) => {
  ...
  let patch = await db
    .collection("users")
    .updateOne({ email: email }, { $set: { status: userStatus } });
...
app.patch("/users/password/:email", async (req, res) => {
  ...
  if (
    (await db
      .collection("users")
      .countDocuments({ email: email, password: old_password })) > 0
  ) {
    let patch = await db
      .collection("users")
      .updateOne(
...
app.patch("/users/info/:email", async (req, res) => {
  ...
  let patch = await db
    ...
    .updateOne(
      { email: email },
      { $set: { name: body.name, surname: body.surname, email: body.email } }
    );
...
app.patch("/tasks/expired", async (req, res) => {
  ...
  let patch = await db
    .collection("tasks")
    .updateOne({ name: name, report: report }, { $set: { status: "expired" } });
  let cursor = await db
    .collection("tasks")
    .find({ name: name, report: report });
  let result = await cursor.toArray();
